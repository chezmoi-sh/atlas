...
#
# Globals:
#   TEST_CONTAINER_ID   The id of the container created
#
# Returns:
...
function container_setup() {
	...
	assert_success
	export TEST_CONTAINER_ID="${lines[0]}"

	until [[ ${wait_seconds} -le 0 || "$(docker inspect -f '{{.State.Health.Status}}' "${TEST_CONTAINER_ID}")" == "healthy" ]]; do
		sleep 1
	...
	-1)
		status=0
		output="container ${__container_name} (${TEST_CONTAINER_ID}) running"
		;;
	0)
		status=1
		output="container ${__container_name} (${TEST_CONTAINER_ID}) is unhealty"
		;;
	*)
		status=0
		output="container ${__container_name} (${TEST_CONTAINER_ID}) running and healty"
		;;
	esac

	# trunk-ignore(shellcheck/SC2317)
	teardown() { container_teardown "${TEST_CONTAINER_ID}"; }
}
