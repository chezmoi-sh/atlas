...
# bats test_tags=docker:compliance
@test "${TEST_APP_NAME} has a valid user configured." {
    run docker image inspect --format '{{ .Config.User }}' "${TEST_APP_IMAGE}"
    assert_success
...
# bats test_tags=docker:compliance
@test "${TEST_APP_NAME} has an healthcheck configured." {
    run docker image inspect --format '{{ .Config.Healthcheck }}' "${TEST_APP_IMAGE}"
    assert_success

    run docker image inspect --format '{{ .Config.Healthcheck.Test }}' "${TEST_APP_IMAGE}"
